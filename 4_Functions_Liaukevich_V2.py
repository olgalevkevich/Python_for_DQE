...
def create_common_dict (input_dict_list) -> dict:
    ...
    # Iterate through the elements of the created list, find identical keys
    ...
    for k in range(len(common_list)):
        ...
        new_key = []
        new_value = []
        for i in range(k, len(common_list) - 1):
            if common_list[k][1] == common_list[i + 1][1]:
        ...
        if flag == 1:  # If the flag is 1, then find the maximum value and the key for this maximum value
            new_key.append(common_list[k][1] + '_' + str(common_list[k][0] + 1))
            new_value.append(common_list[k][2])
            ...
            dict_value = common_list[k][2]
            dict_key = common_list[k][1]
        # Checking whether the original keys were written to the common dictionary.
        if common_list[k][1] not in initial_keys:
            common_dict[dict_key] = dict_value  # if not - add element in common dictionary
            initial_keys.append(common_list[k][1])  # After adding an element to the dictionary, save the original keys for subsequent verification.
    return common_dict
    ...
    for k in range(len(text_list)):
        ...
        for i in range(len(text_list[k])):
            if text_list[k][i].isspace():
                ...
                break
    text_norm_letter_cases += text_list[-1]
...
"""
insert_after_part = 'to the end of this paragraph.'

def new_sentence_insert (given_text, after_part) -> str:
    last_words = ''
    # split sting into sentence strings (separators: .!?) and save it to a list: text_list_str
    text_list_str = re.split(r'[.!?]', given_text)
    for element in text_list_str:
        last_words += ' ' + element.split(' ')[-1]
    ...
    new_text = parts_list[0] + after_part + ' ' + last_words.strip().capitalize()  + '.' + parts_list[1]
    return new_text
